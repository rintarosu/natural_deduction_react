...
※ページを開いてすぐは動作が重いです。気を長くして待ってください。
...
### 遊び方
1.証明した論理式を目標論理式に入力してください。解析ボタンを押すと、論理式の構文を分析して、正しい論理式か判定してくれます。その後、「証明に追加」を押すと目標論理式に設定できます。
2.使いたい前提を新しい前提の入力フォームから入力してください。解析ボタンを押して、再度「証明に追加」ボタンを押してください。
3.証明の行を選択して、ページ下にある規則のボタンを選択してください。規則が正しく適用されれば新たな行が証明に追加されます。
4.目標の論理式と同じ論理式が証明できたらクリアです。
...
- **∨**(選言): …または…
  例: Aを「９月は夏である」、Bを「９月は秋である」とすると、A∨Bは「９月は夏である、または９月は秋である」

- **→**(含意): …ならば…
　例: Aを「明日雨が降る」、Bを「運動会は中止である」とすると、A→Bは「明日雨が降るならば、運動会は中止である」
</details>
